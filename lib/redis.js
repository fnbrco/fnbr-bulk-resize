...
class Redis extends EventEmitter {
    ...
    constructor() {
        ...
        this.subscriber.on('connect', () => {
            this.subscriber.subscribe(config.channel).then((err, c) => {
                logger.debug('Subscribed to ' + config.channel + ' redis events [' + err + ' | ' + c + ']');
            });
        });
                ...
                if(json.event == 'new-upload') {
                    ...
                    MongoDB.get('Items').findOne({ _id: new MongoDB.objId(json.item) }, (err, item) => {
                        ...
                        resizer.resize(item, json.uploadType).then(() => {
                            ...
                            MongoDB.get('Items').updateOne({_id: item._id}, {'$set' : {'images.resizeAvailable' : true}}, (err) => console.error(err));
                        }).catch((rerr) => {
