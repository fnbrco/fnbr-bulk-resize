        ...
        this.subscriber.on('message', (channel, message) => {
            if(channel == config.channel) {
                ...
                if(json.event == 'new-upload') {
                    ...
                    MongoDB.get(config.items.mongoCollection).findOne({ _id: new MongoDB.objId(json.item) }, (err, item) => {
                        if(config.items.ignore.type.indexOf(item.type) >= 0 || config.items.ignore.rarity.indexOf(item.rarityPower) >= 0) {
                            logger.info('[REDIS] Ignoring request for ' + item.name + ' - matched filter');
                            return;
                        }
                        ...
                        resizer.resize(item, json.uploadType).then(() => {
                            ...
                            MongoDB.get(config.items.mongoCollection).updateOne({_id: item._id}, {'$set' : {'images.resizeAvailable' : true}}, (errr) => logger.error(errr));
                            ...
                            MongoDB.get(config.items.mongoCollection).updateOne({_id: item._id}, {'$set' : {'images.resizeAvailable' : false}}, (errr) => logger.error(errr));
                            this.use().del('fnbr_items:' + item._id);
                            this.use().del('fnbr_items:' + item.type + ':' + item.slug);
